...
module.exports.process = async (event, token, entities) => {
  ...
  if (entities.length != 0) {
    ...
    throw new Error(
      "Error fetching *Assignee Issues* because Luis couldn't figure out who you want to know about"
    );
  }

  // Get more information about the user via from Slack
  SlackResponse = await SlackService.GetFullName(
    entity,
    entityType,
    token
  ).catch(error => {
    throw new Error(
      `Error in *Assignee Issues* while trying to convert ${entity}\n${error}`
    );
  });
  ...
  if (
    ...
    SlackResponse["jiraUsername"] == null
  ) {
    throw new Error(
      "Error fetching *Assignee Issues*. The user you specified doesn't have enough name information present in Slack"
    );
  }
  ...
  if (SlackResponse["jiraUsername"]) {
    JiraResponse = await callJira(
      "assignee=" +
        SlackResponse["jiraUsername"] +
        " and status='in progress' ORDER BY updated DESC"
    );
  }
  if (
    (!JiraResponse || JiraResponse["warningMessages"]) &&
    SlackResponse["fullName"]
  ) {
    JiraResponse = await callJira(
      "assignee=" +
        SlackResponse["fullName"] +
  ...
  if (!JiraResponse || JiraResponse["warningMessages"]) {
    throw new Error(
      `Error fetching *Assignee Issues* because neither '${SlackResponse["jiraUsername"]}' nor ${SlackResponse["fullName"]} exist in JIRA`
    );
  }
