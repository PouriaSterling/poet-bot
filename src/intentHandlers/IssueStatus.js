...
module.exports.process = async (event, token, entities) => {
  ...
  if (entities.length != 0) {
    ...
    issueID = await ContextService.fetchContextIssue(
      ...
      throw new Error(
        "Error fetching context issue for *Issue Status*:\n" + error.message
      );
    });
    if (issueID === "none" || issueID === "tooOld") {
      throw new Error(
        "Error fetching *Issue Status* because Luis couldn't find an entity and no issue has been recently discussed"
      );
    }
  }
  ...
  const jiraResponse = await JiraService.issueInfo(issueID).catch(error => {
    throw new Error(
      `Error fetching *Issue Status* of *${issueID}* from JIRA:\n${error.message}`
    );
  });
