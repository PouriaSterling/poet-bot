...
module.exports.process = async (event, token, entities) => {
  ...
  if (entities.length != 0) {
    ...
    throw new Error(
      "Error *Setting Project* because Luis couldn't find a project key"
    );
  }

  // Call JIRA to make sure projectKey is a valid JIRA project
  await JiraService.projectInfo(projectKey).catch(error => {
    throw new Error(`Error *Setting Project*:\n${error.message}`);
  });

  const ContextResponse = await DBService.retrieveChannelContext(
    event.channel
  ).catch(error => {
    throw new Error(
      `Failed to fetch project key for channel context: ${error}`
    );
  });
  ...
  switch (event.actions[0].value) {
    ...
    case "no":
      return SlackService.updateMessage(
        event.channel.id,
        `You've chosen to not update the channel project`,
        [{}],
        token,
        event.message_ts
      );
    ...
    default:
      throw new Error(
        `Internal error: '${event.actions[0].value}' is not a correct option for SetProject interactiveCallback`
      );
  }
};
...
const setProjectKey = async (projectKey, channel, token, timestamp) => {
  ...
  await DBService.updateChannelContext(channel, {
    ...
    throw new Error(
      `Failed to store project key for channel context: ${error}`
    );
  });
